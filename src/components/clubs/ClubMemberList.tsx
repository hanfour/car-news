...
export function ClubMemberList({ slug, isOwner, isAdmin: isAdminProp }: ClubMemberListProps) {
  ...
  const { members, isLoading, mutate } = useClubMembers(slug)
  const [showInviteModal, setShowInviteModal] = useState(false)

  // Auto-detect admin role from fetched members data (avoids extra API call from parent)
  const myRole = user ? members.find((m: ClubMember) => m.user_id === user.id)?.role : undefined
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    try {
      ...
      console.error('[ClubMemberList] handleRoleChange:', err)
    }
  }
  ...
  const handleKick = async (userId: string) => {
    ...
    try {
      ...
      console.error('[ClubMemberList] handleKick:', err)
    }
  }
  ...
  const handleTransferOwnership = async (newOwnerId: string, displayName: string) => {
    ...
    try {
      ...
      console.error('[ClubMemberList] handleTransferOwnership:', err)
    }
  }
  ...
  // Group by role
  const owners = members.filter((m: ClubMember) => m.role === 'owner')
  const admins = members.filter((m: ClubMember) => m.role === 'admin')
  const regularMembers = members.filter((m: ClubMember) => m.role === 'member')

  const renderGroup = (title: string, groupMembers: ClubMember[]) => {
